...
class AudioPlayerProvider extends ChangeNotifier {
  ...
  void _rearmFreeSecondsTicker({bool retryIfNotReady = false}) {
    // Если появились минуты — здесь же гарантированно выходим из ad-mode и снимаем таймер
    _stopAdIfHasMinutes(reason: 'rearmFreeSecondsTicker');
  ...
  void _ensureCreditsConsumer() {
    ...
    if (_creditsConsumer == null) {
      _creditsConsumer = CreditsConsumer(
        ...
        isFreeUser: () => _userType == UserType.free && !_adMode, // в ad-mode не списываем
        onBalanceUpdated: (secLeft, minLeft) {
          // Сервер — истина. Жёстко выставляем остаток.
          setFreeSeconds?.call(secLeft < 0 ? 0 : secLeft);

          // Если снова появились секунды — выходим из ad-mode и убираем таймер рекламы.
          if (secLeft > 0) {
            _log('balance>0 → disable ad-mode & cancel ad timer');
            _disableAdMode();
            cancelAdTimer(reason: 'balance_updated_has_minutes');
            _syncAdScheduleWithPlayback(); // на всякий случай
          }
        },
        onExhausted: () async {
  ...
  Map<String, dynamic>? _normalizeProfile(dynamic raw) {
    ...
    if (raw is User) {
      ...
      try {
        final dyn = raw as dynamic;
        if (dyn.current_listen != null) out['current_listен'] = dyn.current_listen;
        if (dyn.currentListen != null) out['current_listен'] = dyn.currentListen;
  ...
  Future<void> play() async {
    ...
    if (_userType == UserType.free) {
      ...
      // Если секунды есть — страховочно отключим ad-mode и таймер (на случай висящего)
      if (secondsLeft > 0) {
        if (_adMode) _log('play(): has minutes → disable ad-mode');
        _disableAdMode();
        cancelAdTimer(reason: 'play_has_minutes');
      }
  ...
  Future<void> pause() async {
    ...
    _serverPushTimer?.cancel();
    _stopFreeSecondsTicker();
    cancelAdTimer(reason: 'pause'); // === AD-MODE (заменили _stopAdTimer)
    _saveProgressThrottled(force: true);
    await _pushProgressToServer(force: true, allowZero: false);
    notifyListeners();
  }
  ...
  Future<void> stop() async {
    ...
    _serverPushTimer?.cancel();
    _stopFreeSecondsTicker();
    cancelAdTimer(reason: 'stop'); // === AD-MODE (заменили _stopAdTimer)
    _saveProgressThrottled(force: true);
    await _pushProgressToServer(force: true, allowZero: false);
    notifyListeners();
  }
  ...
  /// ПУБЛИЧНО: отменить текущий таймер рекламы (для вызовов снаружи).
  void cancelAdTimer({String reason = ''}) {
    if (_adTimer != null) {
      _log('cancelAdTimer (reason=$reason)');
      _adTimer?.cancel();
      _adTimer = null;
    }
  }

  /// ПУБЛИЧНО: запланировать показ рекламы через d (игнорируется, если есть минуты/не ad-mode).
  void scheduleAdIn(Duration d) {
    final secs = getFreeSeconds?.call() ?? 0;
    if (!_adMode || _userType != UserType.free || secs > 0) {
      _log('scheduleAdIn: skip (adMode=$_adMode, userType=$_userType, freeSeconds=$secs)');
      return;
    }
    // Сдвигаем якорь так, чтобы следующая _scheduleNextAd сработала через d
    _lastAdAt = DateTime.now().subtract(_adInterval - d);
    _syncAdScheduleWithPlayback();
  }
  ...
  // Если есть минуты — выходим из ad-mode и снимаем таймер (страховочный гейт).
  void _stopAdIfHasMinutes({String reason = 'has_minutes'}) {
    final secs = getFreeSeconds?.call() ?? 0;
    if (secs > 0) {
      if (_adMode) _log('$reason → minutes=$secs → disable ad-mode');
      _disableAdMode();
      cancelAdTimer(reason: reason);
    }
  }

  void _syncAdScheduleWithPlayback() {
    // Если minutes > 0 — никакой рекламы по таймеру
    _stopAdIfHasMinutes(reason: 'syncAdSchedule');

    if (_adMode && player.playing) {
      _scheduleNextAd();
    } else {
      cancelAdTimer(reason: 'sync_stop'); // вместо _stopAdTimer()
    }
  }

  // Старый приватный стопер больше не используется напрямую,
  // но оставим для совместимости внутренних вызовов.
  void _stopAdTimer() {
    cancelAdTimer(reason: 'legacy_stop');
  }

  void _scheduleNextAd() {
    if (!_adMode) return;

    // На всякий: если появились минуты прямо перед планированием
    final secs = getFreeSeconds?.call() ?? 0;
    if (secs > 0) {
      _stopAdIfHasMinutes(reason: 'schedule_guard');
      return;
    }
    ...
    _adTimer = Timer(delay, () async {
      // показываем рекламу только если всё ещё играем
      if (_adMode && _isPlayingAudibly()) {
        try {
        ...
        _lastAdAt = DateTime.now(); // чтобы не стрелял мгновенно после паузы
      }
      if (_adMode && player.playing) {
